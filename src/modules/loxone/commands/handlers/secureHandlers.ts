 ...
 */
export function createSecureHandlers(config: LoxoneHttpConfig) {
  return {
    ...
    infoPairing: createHandler((command) => ({
      ...
      payload: {
        ...
        error: -81,
        master: resolveMasterIdentifier(),
        peers: [],
      },
      raw: true,
    })),
...
function resolveMasterIdentifier(): string {
  try {
    const serial = getSystemConfig().miniserver.serial?.trim();
