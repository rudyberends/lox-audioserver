...
import type { CommandResult, HandlerFn } from '@/modules/loxone/commands/types';
import { decodeBase64Segment, safeJsonParse } from '@/modules/loxone/commands/utils/payload';
import { loxoneConfigService } from '@/modules/loxone/services/loxoneConfigService';
import { createLogger } from '@/core/logging/logger';
import { buildSpotifyAuthLink, deleteSpotifyAccount } from '@/modules/content/providers/spotify/serviceAuth';
...
export function createConfigHandlers(config: LoxoneHttpConfig, options: ConfigHandlerOptions = {}) {
  ...
  return {
    ...
    getConfig: handler((command) => {
      const info = loxoneConfigService.getCurrentConfigInfo();
    ...
    setConfig: handler(async (command) => {
      ...
      const crc32 = await loxoneConfigService.setRawAudioConfig({
        raw: parsed,
        rawString: decoded,
      });
    ...
    setVolumes: handler(async (command) => {
      ...
      const count = await loxoneConfigService.applyVolumePreset(parsed.players);
    ...
    setDefaultVolume: handler(async (command) => {
      ...
      await loxoneConfigService.applyDefaultVolume(zoneId, volume);
      return buildResponse(command, 'setdefaultvolume', {
        success: true,
        value: volume,
      });
    ...
    setMaxVolume: handler(async (command) => {
      ...
      await loxoneConfigService.applyMaxVolume(zoneId, volume);
      return buildResponse(command, 'setmaxvolume', {
        success: true,
        value: volume,
      });
    ...
    playerName: handler(async (command) => {
      ...
      try {
        ...
        await loxoneConfigService.applyPlayerNames(updates);
    ...
    serviceCfgDelete: handler(async (command) => {
      ...
      if (service === 'spotify' && userId) {
        await deleteSpotifyAccount(userId);
        // notify frontend to refresh service state
        loxoneConfigService.notifyReloadMusicApp('userdel', 'spotify', userId);
      }
