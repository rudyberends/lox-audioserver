...
export class LoxoneUdpDiscovery {
  ...
  public start(config: LoxoneHttpConfig, port: number = DEFAULT_PORT): void {
    if (this.socket) {
      return;
    }
    ...
    this.responsePayload = Buffer.from(
      this.buildPayload(config, this.fixedTv, this.fixedDi),
      'utf8',
    );
  ...
  private buildPayload(config: LoxoneHttpConfig, tv: string, di: string): string {
