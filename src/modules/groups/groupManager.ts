...
import type { AudioSyncEventPlayer } from '@/modules/groups/types/audioSyncEventPlayer';
import type { AudioSyncGroupPayload } from '@/modules/groups/types/AudioSyncGroupPayload';
...
export class GroupManager {
      ...
      switch (event) {
        ...
        case 'remove': {
          ...
          if (record?.members?.length) {
            void airplayGroupController.stopGroupMembers(record.leader, record.members);
            const membersToStop = record.members.filter((memberId) => memberId !== record.leader);
            membersToStop.forEach((memberId) => {
              try {
                zoneManager.handleCommand(memberId, 'stop');
              } catch (error) {
  ...
  public broadcastGroupState(): void {
    ...
    broadcastMessage(JSON.stringify({ audio_sync_event: payload }));
  ...
  private syncAirplayGroup(leader: number): void {
    ...
    void airplayGroupController.syncCurrentGroup(leader);
  }

  private buildGroupPayload(record: GroupRecord): AudioSyncGroupPayload | null {
    const leaderState = zoneManager.getState(record.leader);
    if (!leaderState) {
      return null;
    }
  ...
  public async applyMasterVolume(zoneId: number, target: number): Promise<void> {
    ...
    const leaderState = zoneManager.getState(group.leader);
    if (!leaderState) {
    ...
    for (const memberId of memberIds) {
      const state = zoneManager.getState(memberId);
      if (!state) {
  ...
  public applySpecGroupVolume(zoneId: number, target: number): void {
    ...
    const players = [...memberIds].map((id) => {
      const state = zoneManager.getState(id);
  ...
  private setZoneVolume(zoneId: number, volume: number): void {
    try {
      zoneManager.handleCommand(zoneId, 'volume_set', String(volume));
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      ...
      if (previous) {
        ...
        prevMembers.forEach((memberId) => {
          ...
          if (!nextMembers.has(memberId)) {
            void airplayGroupController.stopGroupMembers(record.leader, [memberId]);
            try {
              zoneManager.handleCommand(memberId, 'stop');
            } catch (error) {
              const message = error instanceof Error ? error.message : String(error);
