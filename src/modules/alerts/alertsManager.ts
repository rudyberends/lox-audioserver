...
import { FileAlertProvider } from '@/modules/alerts/fileAlertProvider';
import { GoogleTtsProvider } from '@/modules/alerts/googleTtsProvider';
import type { AlertAction, AlertActionResult, AlertMediaResource } from '@/modules/alerts/types';
...
export class AlertsManager {
  ...
  public async handleGroupedAlert(
    ...
    if (action === 'off') {
      await Promise.all(zones.map((zoneId) => zoneManager.stopAlert(zoneId)));
    ...
    await Promise.all(
      zones.map(async (zoneId) => {
        ...
        await zoneManager.startAlert(zoneId, normalizedType, media, volume);
      }),
    );
  ...
  public async handleUploadedAlert(
    ...
    await Promise.all(
      zones.map(async (zoneId) => {
        ...
        await zoneManager.startAlert(zoneId, 'uploaded', media, volume);
      }),
    );
  ...
  private resolveAlertVolume(zoneId: number, type: string): number {
    const volumes = zoneManager.getZoneVolumes(zoneId);
    if (!volumes) {
