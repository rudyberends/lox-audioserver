...
import { resolveLineInIngestResampler, resolveLineInSampleRate } from '@/modules/audio/inputs/linein/lineInConstants';
  ...
  public async handle(
    ...
    if (bridgeStatusMatch) {
      ...
      if ((body as BridgeStatusPayload).track_change === true) {
        ...
        if (inputId) {
          lineInMetadataService.handleTrackChange(inputId);
        }
      }
