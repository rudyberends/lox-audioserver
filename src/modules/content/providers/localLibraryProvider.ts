...
import type { SearchLimits } from '@/modules/content/utils/searchLimits';
import { ensureNasMounts, listStorages } from '@/modules/content/storage/storageManager';
import { getSystemConfig } from '@/domain/config/configStore';
import { notifyRescan } from '@/modules/loxone/ws/notifier';
import {
  LocalLibraryStore,
  type AlbumRow,
  type AlbumCoverRow,
  type ArtistRow,
  type StoredTrack,
} from '@/modules/content/providers/localLibraryStore';
...
export class LocalLibraryProvider {
  ...
  private buildCoverUrl(track: { relPath: string; cover?: string | null }): string {
    ...
    const host = getSystemConfig().audioserver.ip || '127.0.0.1';
  ...
  private updateScanStatus(status: ScanStatus, silent: boolean, stats?: LibraryStats): void {
    ...
    if (!silent) {
      notifyRescan(status, stats?.albums, stats?.tracks);
    }
  }
