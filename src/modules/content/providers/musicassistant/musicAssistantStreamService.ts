...
import { decodeAudiopath, encodeAudiopath } from '@/modules/audio/utils/audiopath';
import { PassThrough } from 'node:stream';
import { notifyTransportError, updateQueueFromTransport } from '@/modules/audio/outputs/queueUpdater';
import { generateQueueId } from '@/modules/zones/helpers/queueHelpers';
...
class MusicAssistantStreamService {
  ...
  public async startStreamForAudiopath(
    ...
    if (maConfig?.offload) {
      ...
      try {
        ...
        return { playbackSource: null, transportOnly: true };
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
  ...
  private async handleQueueEvent(zoneId: number, playerId: string, evt: Record<string, any>): Promise<void> {
    ...
    updateQueueFromTransport(zoneId, mapped, currentIndex);
  }
...
export const musicAssistantStreamService = new MusicAssistantStreamService();
