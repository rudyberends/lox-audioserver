...
import { consumePkceVerifier } from '@/modules/content/providers/spotify/pkce';
import { resolveSpotifyClientId } from '@/modules/content/providers/spotify/utils';
import {
  generateLibrespotCredentialsFromOAuth,
} from '@/modules/content/providers/spotify/spotifyStreamingService';
import fsp from 'node:fs/promises';
import path from 'node:path';
import { createPkcePair } from '@/modules/content/providers/spotify/pkce';
import { notifyReloadMusicApp } from '@/modules/loxone/ws/notifier';
import { getSharedSpotifyManager } from '@/modules/content/providers/spotifyServiceManager';
import {
  spotifyInputService,
  pushLibrespotCredentials,
} from '@/modules/audio/inputs/spotify/spotifyInputService';
...
export function buildSpotifyAuthLink(params: BuildLinkParams): string {
  const cfg = getConfig();
...
export async function handleSpotifyOAuthCallback(
  ...
  try {
    ...
    if (accessToken) {
      try {
        ...
        if (creds?.credentials) {
          ...
          await pushLibrespotCredentials(accountId, parsed);
    ...
    notifyReloadMusicApp('useradd', 'spotify', accountId);
    reinitializeSpotifyInputs('account_created', accountId);
...
export async function handleSpotifyLibrespotOAuth(
  ...
  const spotifyManager = getSharedSpotifyManager();
  ...
  try {
    ...
    reinitializeSpotifyInputs('oauth_credentials_updated', accountId);
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
...
export async function deleteSpotifyAccount(userId: string): Promise<void> {
  ...
  contentManager.refreshFromConfig();
  notifyReloadMusicApp('userdel', 'spotify', userId);
  reinitializeSpotifyInputs('account_deleted', userId);
}
...
async function persistAccount(account: SpotifyAccountConfig): Promise<void> {
  await updateConfig((cfg) => {
    const accounts = cfg.content.spotify.accounts ?? [];
...
function reinitializeSpotifyInputs(reason: string, accountId?: string): void {
  try {
    const cfg = getConfig();
    ...
    if (accountId) {
      ...
      if ((account as any)?.librespotCredentials) {
        pushLibrespotCredentials(accountId, (account as any).librespotCredentials).catch(
          (error) => {
            const message = error instanceof Error ? error.message : String(error);
