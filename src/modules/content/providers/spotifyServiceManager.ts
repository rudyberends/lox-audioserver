...
import { TidalProvider } from '@/modules/content/providers/tidal/tidalProvider';
...
function loadSpotifyConfig(): { accounts: SpotifyAccountConfig[]; clientId?: string; bridges: SpotifyBridgeConfig[] } {
  try {
    const cfg = getConfig();
    return normalizeSpotifyConfig(cfg.content?.spotify);
  } catch {
    try {
      const path = resolveDataDir('config.json');
...
export function buildSpotifyManagerFromConfig(): SpotifyServiceManager {
  const cfg = loadSpotifyConfig();
  return new SpotifyServiceManager(cfg.accounts, cfg.clientId, cfg.bridges);
}

let sharedManager: SpotifyServiceManager | null = null;

/**
 * Returns a shared Spotify manager instance so refresh tokens are reused across transports/providers.
 */
export function getSharedSpotifyManager(): SpotifyServiceManager {
  if (!sharedManager) {
    sharedManager = buildSpotifyManagerFromConfig();
  }
  return sharedManager;
}

/**
 * Reloads the shared Spotify manager with the latest config while keeping token cache in-memory.
 */
export function reloadSharedSpotifyManager(): SpotifyServiceManager {
  const cfg = loadSpotifyConfig();
  if (sharedManager) {
    sharedManager.reload(cfg.accounts ?? [], cfg.clientId, cfg.bridges ?? []);
    return sharedManager;
  }
  sharedManager = new SpotifyServiceManager(cfg.accounts ?? [], cfg.clientId, cfg.bridges ?? []);
  return sharedManager;
}
