...
import {
  getSpotifyDeviceId,
  setSpotifyDeviceId,
  clearSpotifyDeviceId,
} from '@/modules/audio/outputs/spotify/deviceRegistry';
import { updateConfig } from '@/domain/config/configStore';
import {
  updateQueueFromTransport,
  notifyTransportError,
  notifyTransportState,
} from '@/modules/audio/outputs/queueUpdater';
import type { QueueItem } from '@/modules/zones/zoneManager';

export const SPOTIFY_CONNECT_CONTROLLER_DEFINITION: TransportConfigDefinition = {
  id: 'spotify',
...
interface SpotifyConnectTransportConfig {
  deviceName?: string;
  deviceId?: string;
}
...
export class SpotifyConnectInputController implements ZoneTransport {
  // Acts as an input/controller, not an output transport for selection.
  ...
  constructor(
    ...
    config: SpotifyConnectTransportConfig,
  ) {
  ...
  public async play(session: PlaybackSession): Promise<void> {
    ...
    const now = Date.now();
    const suppressTransportError = this.isPipeSession(session);
    const uri = targetUri;
    ...
    if (!target && (suppressTransportError || isAirplay)) {
    ...
    if (!target) {
      ...
      if (!suppressTransportError) {
        notifyTransportError(this.zoneId, 'uri');
      }
      return;
    }
    ...
    if (!auth) {
      ...
      if (!suppressTransportError) {
        notifyTransportError(this.zoneId, 'auth');
      }
      return;
    }
    let deviceId = this.preferredDeviceId ?? getSpotifyDeviceId(this.zoneId);
    if (deviceId) {
    ...
    if (!deviceId) {
      ...
      if (deviceId) {
        setSpotifyDeviceId(this.zoneId, deviceId);
    ...
    if (!deviceId) {
      ...
      if (!suppressTransportError) {
        notifyTransportError(this.zoneId, 'device');
      }
      return;
    }
    ...
    deviceId = activated;
    const ok = await this.transferAndPlay(auth.token, deviceId, body, suppressTransportError);
    if (!ok) {
      if (!suppressTransportError) {
        notifyTransportError(this.zoneId, 'device');
      }
  ...
  public async pause(session: PlaybackSession | null): Promise<void> {
    ...
    const suppressTransportError = false;
    ...
    this.lastAccountId = auth.accountId;
    const deviceId =
      (await this.ensureActiveDevice(auth.accountId, auth.token, undefined, true, suppressTransportError)) ||
    ...
    if (res !== null) {
      notifyTransportState(this.zoneId, { status: 'paused' });
      void this.validatePlayback(auth.token, auth.accountId, this.lastPlayUri ?? '', suppressTransportError, true);
    }
  ...
  public async resume(session: PlaybackSession | null): Promise<void> {
    ...
    const ok = await this.transferAndPlay(auth.token, deviceId, {}, suppressTransportError);
    if (ok) {
      if (!this.isPipeSession(session)) {
        this.pollingEnabled = true;
      ...
      notifyTransportState(this.zoneId, { status: 'playing' });
    }
  }

  public async stop(session: PlaybackSession | null): Promise<void> {
    // Do not issue Spotify pauses on transport-wide stop (often error-driven); just stop local polling.
    if (session === null) {
  ...
  private publishLocalQueueState(uri: string, session: PlaybackSession): void {
    ...
    updateQueueFromTransport(this.zoneId, [item], 0);
  }
  ...
  private scheduleValidation(token: string, accountId: string, targetUri: string, suppressTransportError = false): void {
    if (!this.pollingEnabled) {
    ...
    this.validationTimer = setTimeout(() => {
      this.validationTimer = null;
      void this.validatePlayback(token, accountId, targetUri, suppressTransportError, false);
  ...
  private async validatePlayback(
    ...
    if (!snapshot) {
      ...
      this.scheduleValidation(token, accountId, targetUri, suppressTransportError);
      return;
    }
    ...
    if (mapped) {
      ...
      updateQueueFromTransport(this.zoneId, [mapped], 0);
      const position =
    ...
    void this.syncQueue(token, accountId, suppressTransportError);
  ...
  private async resolveAuth(accountHint?: string | null): Promise<{
    ...
    let manager = this.spotify ?? getSharedSpotifyManager();
    this.spotify = manager;
    let accountId = this.pickAccountId(accountHint, manager);
    if (!accountId) {
      manager = reloadSharedSpotifyManager();
      this.spotify = manager;
  ...
  private pickAccountId(
    ...
    this.log.warn('no matching spotify account found for transport', {
      zoneId: this.zoneId,
      hint: accountHint,
  ...
  private async ensureActiveDevice(
    ...
    push(getSpotifyDeviceId(this.zoneId));

    const devices = await this.apiRequest<{
      devices: Array<{ id: string; name?: string; type?: string; is_active?: boolean }>;
    }>('GET', '/devices', token, undefined, undefined, undefined, suppressTransportError);
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (candidate && candidate === getSpotifyDeviceId(this.zoneId) && candidate !== this.preferredDeviceId) {
    ...
    if (!suppressTransportError && warnTs - this.lastNoDeviceWarnAt > this.noDeviceWarnIntervalMs) {
  ...
  private async resolveDeviceId(
    ...
    const learnedId = getSpotifyDeviceId(this.zoneId);
    if (learnedId) {
    ...
    if (devices?.devices?.length) {
      ...
      if (match?.id) {
        setSpotifyDeviceId(this.zoneId, match.id);
  ...
  private async clearPersistedDeviceId(deviceId: string): Promise<void> {
    clearSpotifyDeviceId(this.zoneId);
    try {
      await updateConfig((config) => {
        const zone = config.zones.find((z) => z.id === this.zoneId);
  ...
  private async syncQueue(token: string, accountId?: string, suppressTransportError = true): Promise<void> {
    if (accountId) {
    ...
    updateQueueFromTransport(this.zoneId, items, 0);
  }
  ...
  private async apiRequest<T>(
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
      try {
        ...
        const logFn = suppressTransportError ? this.log.debug.bind(this.log) : this.log.warn.bind(this.log);
        ...
        if (!suppressTransportError) {
          notifyTransportError(this.zoneId, 'error');
        }
        return null;
      }
    }
  ...
  private async transferAndPlay(
    ...
    await this.waitForActiveDevice(token, deviceId, 1500, suppressTransportError);
    ...
    if (res === null) {
      ...
      await this.waitForActiveDevice(token, deviceId, 1500, suppressTransportError);
      res = await this.apiRequest('PUT', endpointWithDevice, token, body, deviceId, true, suppressTransportError);
    }
    return res !== null;
  }
