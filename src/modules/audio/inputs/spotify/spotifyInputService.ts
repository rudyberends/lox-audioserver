...
import { ensureInputs, getConfig, updateConfig } from '@/domain/config/configStore';
import type {
  GlobalSpotifyConfig,
...
export interface SpotifyConnectController {
  startPlayback(
    zoneId: number,
    label: string,
...
class SpotifyConnectInstance {
  ...
  public async start(): Promise<void> {
    ...
    const deviceId = getSpotifyDeviceId(this.zoneId) ?? this.deviceId ?? `lox-zone-${this.zoneId}`;
    this.deviceId = deviceId;
    const publishName = this.config.publishName || this.zoneName;

    const manager = getSharedSpotifyManager();
  ...
  private resolveClientId(accountId?: string): string | null {
    const cfg = getConfig()?.content?.spotify;
  ...
  private handleNativeEvent(ev: any): void {
    ...
    if (typeRaw === 'error') {
      ...
      notifyTransportError(this.zoneId, `spotify ${message}`);
      this.stopConnectHost();
      this.scheduleRestart();
      return;
    }
  ...
  private startControllerPlayback(metadata: PlaybackMetadata): void {
    this.isActive = true;
    try {
      airplayInputService.stopActiveSession(this.zoneId, 'switch_to_spotify');
    } catch {
      /* ignore */
    }
...
class SpotifyInputService {
  ...
  public syncZones(zones: ZoneConfig[], spotifyConfig?: GlobalSpotifyConfig | null): void {
    ...
    if ((!accounts || accounts.length === 0) && getConfig()?.content?.spotify?.accounts) {
      accounts = getConfig().content.spotify.accounts;
    }
    ...
    for (const zone of zones) {
      ...
      setSpotifyDeviceId(zone.id, ensuredDeviceId);
      ...
      if (connectEnabled) {
        ...
        if (!offloadEnabled) {
          instance.start().catch(() => undefined);
        }
        instance.stopConnectHost();
      }
    }
...
export const spotifyInputService = new SpotifyInputService();

export async function pushLibrespotCredentials(
  accountId: string,
  credentials: string | Record<string, unknown>,
): Promise<void> {
  await spotifyInputService.applyLibrespotCredentials(accountId, credentials);
}
