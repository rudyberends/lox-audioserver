...
import { waitForStreamRequest } from '@/modules/http/streams/streamEvents';

export interface DlnaTransportConfig {
  host?: string;
  controlUrl?: string;
}

export const DLNA_TRANSPORT_DEFINITION: TransportConfigDefinition = {
  id: 'dlna',
...
export class DlnaTransport implements ZoneTransport {
  public readonly type = 'dlna';
  private readonly log = createLogger('Transport', 'DLNA');
  ...
  constructor(
    ...
    if (typeof config.controlUrl === 'string' && config.controlUrl.trim().length > 0) {
      ...
      this.log.info('DLNA transport configured with manual control URL', {
        zoneId: this.zoneId,
        zone: this.zoneName,
        controlUrl: this.controlUrl,
      });
    } else if (this.host) {
      this.log.info('DLNA transport awaiting discovery', {
        zoneId: this.zoneId,
        zone: this.zoneName,
        host: this.host,
      });
    } else {
      this.log.warn('DLNA transport has no host or control URL configured', {
        zoneId: this.zoneId,
        zone: this.zoneName,
      });
    }
  ...
  public async play(session: PlaybackSession): Promise<void> {
    if (!session.playbackSource) {
      this.log.debug('DLNA transport skipped', { zoneId: this.zoneId, source: session.source });
      return;
    }
  ...
  private async sendPlaybackWithSoap(uri: string): Promise<void> {
    ...
    if (!didSetUri) {
      if (timedOut) {
        ...
        const seen = await waitForStreamRequest({
          zoneId: this.zoneId,
          host: this.host,
          timeoutMs: 12000,
        });
