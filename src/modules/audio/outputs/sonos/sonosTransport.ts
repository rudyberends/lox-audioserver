...
import { SonosClient } from '@lox-audio-server/sonos';
...
import { sonosGroupController } from '@/modules/audio/outputs/sonos/sonosGroupController';
import { notifyTransportError } from '@/modules/audio/outputs/queueUpdater';
import { discoverSonosDevice } from '@/modules/audio/outputs/sonos/sonosDiscovery';

export interface SonosTransportConfig {
  host?: string;
  controlUrl?: string;
...
export const SONOS_TRANSPORT_DEFINITION: TransportConfigDefinition = {
  id: 'sonos',
...
export class SonosTransport implements ZoneTransport {
  public readonly type = 'sonos';
  private readonly log = createLogger('Transport', 'Sonos');
  ...
  constructor(
    ...
    if (typeof config.controlUrl === 'string' && config.controlUrl.trim().length > 0) {
      ...
      this.log.info('Sonos transport configured with manual control URL', {
        zoneId: this.zoneId,
        zone: this.zoneName,
        controlUrl: this.controlUrl,
      });
    } else if (this.host) {
      this.log.info('Sonos transport awaiting discovery', { zoneId: this.zoneId, host: this.host });
    } else if (this.autoDiscover) {
      this.log.info('Sonos transport will auto-discover device', { zoneId: this.zoneId });
    } else {
      this.log.warn('Sonos transport has no host or control URL configured', { zoneId: this.zoneId });
    }
    sonosGroupController.register(this.zoneId, this);
  }
  ...
  public async play(session: PlaybackSession): Promise<void> {
    if (!session.playbackSource) {
      this.log.warn('Sonos transport skipped; no playback source', { zoneId: this.zoneId });
      notifyTransportError(this.zoneId, 'sonos no source');
      return;
    }
    if (await sonosGroupController.tryJoinLeader(this)) {
      return;
    }
    const uri = this.resolveStreamUri(session);
    if (!uri) {
      this.log.warn('no playable URI for session', { zoneId: this.zoneId });
      notifyTransportError(this.zoneId, 'sonos no stream uri');
      return;
    }
    ...
    await sonosGroupController.syncGroupMembers(this);
  ...
  public dispose(): void {
    sonosGroupController.unregister(this.zoneId);
    if (this.s2Client) {
...
<DIDL-Lite xmlns="urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/"
  ...
  private normalizeStreamUri(uri: string): string {
    const match = uri.match(/\/streams\/(\d+)\/[^/?#]+\.(mp3|aac)/i);
    if (!match) {
      return uri;
    }
    ...
    const ext = match[2]?.toLowerCase() === 'aac' ? 'aac' : 'mp3';
