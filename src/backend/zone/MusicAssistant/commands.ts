...
export interface MusicAssistantCapability {
  name: string;
...
export function createZoneControlCapability(): MusicAssistantCapability {
  const supported = new Set(['resume', 'play', 'pause', 'stop', 'volume']);
  ...
  return {
    ...
    async execute(ctx, command, param) {
      switch (command) {
        case 'resume':
        case 'play':
        ...
        case 'volume': {
          ...
          if (!zone) return true; // helper already logged
...
export function createZoneGroupingCapability(): MusicAssistantCapability {
  const supported = new Set(['groupJoin', 'groupJoinMany', 'groupLeave', 'groupLeaveMany']);
...
export function createZoneContentPlaybackCapability(): MusicAssistantCapability {
  const supported = new Set(['queue', 'queueplus', 'queueminus', 'repeat', 'shuffle', 'position']);

  return {
    name: 'ZoneContentPlayback',
    handles(command: string) {
      return supported.has(command);
    },
    async execute(ctx, command, param) {
      switch (command) {
        ...
        case 'position': {
          ...
          if (requestedSeconds === undefined) {
            logger.warn(
              `[MusicAssistant][Zone:${ctx.loxoneZoneId}] position command missing numeric payload`,
            );
            return true;
          }
        ...
        case 'queue': {
          ...
          if (action === 'play' && target) {
            try {
              ...
              return true; // command was intended; treat as handled
            }
          }
