import { handler, handleAddExtensionClick } from '../core/events.ts';
import { setupTabs, updateTabs, updateHeroSummary, setProviderOption, updateProviderDiscoveryState, setModalParameter, updateDiscoveryState, setZoneStatusIndicator, clearZoneErrorTimer, setStatus, clearStatus } from '../core/state.ts';
import { render } from '../ui/render.ts';
import { renderPanels } from '../ui/renderPanels.ts';
import { renderStatus } from '../ui/renderStatus.ts';
import { renderPairingBadge } from '../ui/renderPairingBadge.ts';
import { renderPairingWaitIndicator } from '../ui/renderPairingWaitIndicator.ts';
import { renderMiniserverBadge } from '../ui/renderMiniserverBadge.ts';
import { renderZoneGroup } from '../ui/renderZoneGroup.ts';
import { renderZoneCard } from '../ui/renderZoneCard.ts';
import { renderExtensionPlaceholderCard } from '../ui/renderExtensionPlaceholderCard.ts';
import { renderAddExtensionControls } from '../ui/renderAddExtensionControls.ts';
import { renderZonesMetricIcon } from '../ui/renderZonesMetricIcon.ts';
import { renderZonesOverview } from '../ui/renderZonesOverview.ts';
import { renderProviderContent } from '../ui/renderProviderContent.ts';
import { renderProviderField } from '../ui/renderProviderField.ts';
import { renderZoneMetadata } from '../ui/renderZoneMetadata.ts';
import { renderAdapterModal } from '../ui/renderAdapterModal.ts';
import { renderAdapterField } from '../ui/renderAdapterField.ts';
import { renderZonesPanel } from '../ui/zones.ts';
import { renderUpdatePanel } from '../ui/update.ts';
import {
  getConfig,
  saveConfig,
  reloadConfig,
...
async function loadConfig(silent = false) {
  ...
  try {
    ...
    const providerHost = providerHostRaw.  const panelsHtml = renderPanels(state.config);
  cdiv class="miniserver-header">
...
async function refreshZoneStates(silent = false) {
  try {
    ...
    if (!silent)               <div class="pairing-step-heading">
...
function extractExtensionIndex(value) {
  ...
  const match = value.match(/extension\ || 0;
  const highestIndex = Number(extensionStats.highestIndex) || 0;
  const remaining = Math.max(0, MAX_EXTENSION_COUNT - totalCount);
  const nextIndex = highestIndex + 1;
  const canAria-hidden="true">
          <path d="M5 5h4v4H5zM10 10h4v4h-4zM15ss="zones-metric-label">${escapeHtml(metric.label)}</span>
    ...
    <section class="zones-provider-card">
      ...
      <div class="zones-provider-card__body">
        ...
        <aside class="zones-provider-card__summary">
          ...
          <p class="zones-prov     <p class="zones-provider-card__summary-hint">${escapeHtml(guidance)}</p>
        </aside>
      </div>
    </section>
  `;
...
function renderProviderField(field, parameters, discoveryState = {}) {
  ...
  document.querySelectorAll('[data-provider-field]').forEach((element) => {
    ...
    if (!fieldId) return;
    if (element instanceof HTMLInputElement) {element.addEventListener('input', handler);
...
function updateProviderDiscoveryState(fieldId, patch = {}) {
  ...
  const current = state.providerDiscovery[fieldId] ||{
    options: [],
    loading: false,
    error:ZoneMetadata(status = {}, options = {}) {
...
function setModalParameter(fieldId, value) {
  ...
  if (value === '' || value === null || value === undefined) {
    ...
    next[fieldId] = vapdateModalState({ discovery: next });
}
...
function setZoneStatusIndicator(zoneId, className, text) {
  ...
  if (statusEl instanceof HTMLElement) {
    statusEl.setAttribute('aria-hidden', 'trustring' ? audioserver.ip.trim() : '';
