...
export class SnapcastCore {
  ...
  private applyZoneVolumeToClient(clientId: string, streamId: string): void {
    ...
    const zoneVolume = this.zones.getZoneState(active.zoneId)?.volume;
    ...
    switch (method) {
      ...
      case 'Stream.SetProperty': {
        ...
        if (property === 'volume') {
          ...
          this.zones.handleCommand(zoneId, 'volume_set', String(value));
          return reply('ok');
        }
        if (property === 'mute') {
        ...
        if (property === 'shuffle') {
          ...
          this.zones.setShuffle(zoneId, params.value);
          return reply('ok');
        }
  ...
  private buildStreamStatus(active: ActiveStream): any {
    const session = this.audioManager.getSession(active.zoneId);
    const meta = session?.metadata;
    const zoneState = this.zones.getZoneState(active.zoneId);
