...
export class AudioSession {
  ...
  private encoderProc?: ChildProcessWithoutNullStreams;
  ...
  private async inlineCrossfadeFromPipeFFmpeg(
    ...
    if (fadeIn.kind === 'pipe') {
      ...
      this.pipeSourceErrorListener = (err: any) => {
        this.log.warn('crossfade pipe stream error', { zoneId: this.zoneId, message: err?.message || String(err) });
        if (!this.crossfadeActive && !this.ending) this.encoderInput?.end();
      };
